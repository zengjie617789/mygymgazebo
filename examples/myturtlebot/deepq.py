...
class SumTree(object):
    ...
    def __init__(self, capacity,phi_length):
    ...
    def get_leaf(self, v):
        ...
        return leaf_idx, self.tree[leaf_idx],self.data[data_idx+self.phi_length-3]
...
class Memory(object):  # stored as ( s, a, r, s_ ) in SumTree
    ...
    def sample(self,n,phi_length):
        ...
        for i in range(n):
            ...
            b=Data[:,:self.state_size]
            self.States[i]=b.reshape((1,self.state_size*phi_length))
            # self.States[i]=data[0:self.state_size]
            # self.States[i]=self.phi(self.States[i])
            self.actions[i]=Data[-1,self.state_size]
            self.rewards[i]=Data[-1,self.state_size+1]
            self.nextStates[i]=Data[-self.state_size-1:-1].reshape(1,self.state_size*phi_length)
            self.terminals[i]=Data[-1,-1]
...
class AgentTF:
    ...
    # def train(self, stepCounter):
    #     # Train_batch = [s,a,r,s1,d]
    #     #Perform the Double-DQN update to the target Q-values
    #     # tree_idx,ISWeight,States,actions,rewards,nextStates,terminals=self.memory.sample(self.batch_size,self.phi_length)
    #     # States=batch_memory[:,:self.state_size]
    #     # nextStates=batch_memory[:,-self.state_size-1:-1]
    #     # actions=batch_memory[:,self.state_size]
    #     # rewards=batch_memory[:,-self.state_size-1]
    #     # terminals=batch_memory[:,-1]
    #
    #
    #     Q1 = self.sess.run(self.mainQN.predict,
    #                   feed_dict={self.mainQN.input:nextStates})
    #
    #     Q2 = self.sess.run(self.targetQN.Qout,
    #                   feed_dict={self.targetQN.input:nextStates})
    #
    ...
    #     _,loss,abs_errors,summary = self.sess.run([self.mainQN.updateModel, self.mainQN.loss,self.mainQN.td_error,self.mainQN.merged],
    #                  feed_dict={self.mainQN.input:States,
    #                  self.mainQN.targetQ:targetQ,
    #                  self.mainQN.actions:actions,
    #                  self.mainQN.ISWeight:ISWeight})
    #     self.memory.batch_update(tree_idx,abs_errors)
    #     # Set the target network to be equal to the primary
    #     self.update_target_graph(self.target_ops)
    #
    #     self.writer.add_summary(summary,stepCounter)
    #     return loss
